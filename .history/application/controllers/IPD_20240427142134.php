...
//                        $ledger[ 'credit' ]      = $ipd_total - $sale -> deduction;
                ...
                $ledger      = array (
                    ...
                    'credit'           => 0,
                    'debit'            => is_patient_panel ( 0, $patient ) ? $ipd_total : count_ipd_payment_received ( $sale_id ),
                    'description'      => $description,
                    'date_added'       => current_date_time (),
                );
                $this -> AccountModel -> add_ledger ( $ledger );
        ...
        public function addConsultantCommissioningLedger ( $sale_id ) {
            ...
            if ( count ( $consultants ) > 0 ) {
                foreach ( $consultants as $consultant ) {
                    ...
                    $description = 'Cost of Procedure (' . $service -> title . '). IPD Invoice ID# ' . $sale_id;
                    $ledger      = array (
                        'user_id'          => get_logged_in_user_id (),
...
//                }
                ...
                $where = array (
                    'acc_head_id' => $accountHeadID > 0 ? $accountHeadID : cash_from_ipd,
                    'invoice_id'  => $sale_id,
                    'ipd_sale_id' => $sale_id,
                );
