    ...
    class CultureTest extends CI_Controller {
        ...
        public function add_culture_report () {
            ...
            if ( isset( $_GET[ 'sale-id' ] ) and $_GET[ 'sale-id' ] > 0 and isset( $_GET[ 'test-id' ] ) and $_GET[ 'test-id' ] > 0 ) {
                ...
                $report = $this -> RadiologyModel -> get_report_saleId_testId ( $sale_id, $test_id, 'hmis_culture' );
                if ( !empty( $report ) ) {
                    redirect ( '/culture/culture/edit?report_id=' . $report -> id );
                }
            }
        ...
        public function process_add_report ( $POST ) {
            ...
            if ( $id > 0 ) {
                ...
                redirect( 'CultureTest/add_culture_report?disable=true' );
            }
            else {
