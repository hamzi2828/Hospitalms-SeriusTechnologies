...
//    }
    ...
    foreach ($records as $record) {
        ...
        // Accumulate opening and running balances
        $totalOpeningBalance += $opening_balance;
        $totalRunningBalance += $running_balance;
        ...
        if ((isset($record->children) && count($record->children) > 0) || (abs($credit) > 0 || abs($debit) > 0)) {
            ...
            if (!empty($record->children)) {
                ...
                // Accumulate child's opening and running balances
                $totalOpeningBalance += $childResult['totalOpeningBalance'];
                $totalRunningBalance += $childResult['totalRunningBalance'];
            }
        }
    }
    ...
    return array(
        ...
        'totalRunningBalance' => $totalRunningBalance,
    );
}
...
function displayRecursiveAccountHeadsPayableReport($records, $level = 1, $reverseFormula = false) {
    ...
    $netDebit  = 0;
    $netOpeningBalance = 0;
    $netRunningBalance = 0; // Track running balance total
    ...
    foreach ($records as $record) {
        ...
        // Accumulate current record's values
        $netCredit += $credit;
        $netDebit  += $debit;
        $netOpeningBalance += $opening_balance;
        
        // Calculate running balance
        $running_balance = ($opening_balance + $debit) - $credit;
        $netRunningBalance += $running_balance; // Accumulate running balance
        ...
        if ((isset($record->children) && count($record->children) > 0) || (abs($credit) > 0 || abs($debit) > 0)) {
            ...
            if (!empty($record->children)) {
                ...
                // Accumulate child's values
                $netCredit += $childResult['netCredit'];
                $netDebit  += $childResult['netDebit'];
                $netOpeningBalance += $childResult['netOpeningBalance'];
                $netRunningBalance += $childResult['netRunningBalance']; // Add child running balance
            }
        }
    }
    ...
    return array(
        ...
        'netCredit'         => $netCredit,
        'netDebit'          => $netDebit,
        'netOpeningBalance' => $netOpeningBalance,
        'netRunningBalance' => $netRunningBalance // Return running balance total
    );
}
