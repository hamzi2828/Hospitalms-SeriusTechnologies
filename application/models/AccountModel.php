...
//        }
        ...
        public function profit_loss_statement_2($data, $level = 0, $hide_actions = false, &$first_level_sr = 0, &$second_level_sr = '', &$third_level_sr = '', &$fourth_level_sr = '', &$fifth_level_sr = '') {
            ...
            foreach ($data as $row) {
                ...
                if ($has_children) {
                    $html .= $this->profit_loss_statement_2(
                        $row['children'],
                        $level + 1,
                        $hide_actions,
                        $first_level_sr,
        ...
        public function profit_loss_statement_2_new($data, $level = 0, $hide_actions = false, &$first_level_sr = 0, &$second_level_sr = '', &$third_level_sr = '', &$fourth_level_sr = '', &$fifth_level_sr = '') {
            $html = '<tbody>';
            $sr_number = 3;
            $start_date = isset($_GET['start_date']) && !empty(trim($_GET['start_date']))
                ? date('Y-m-d', strtotime($_GET['start_date']))
            ...
            foreach ($data as $row) {
                ...
                $has_children = isset($row['children']) && is_array($row['children']) && count($row['children']) > 0;
                if ($has_children) {
                    $html .= $this->profit_loss_statement_2_new(
                        $row['children'],
                        $level + 1,
                        $hide_actions,
                        $first_level_sr,
        ...
        public function build_chart_of_accounts_table_for_Balance_summary_pro_2($data, $level = 0, $hide_actions = false, &$first_level_sr = 0, &$second_level_sr = '', &$third_level_sr = '', &$fourth_level_sr = '', &$fifth_level_sr = '') {
            ...
            foreach ($data as $row) {
                ...
                // Check if this is the Income/Sales/Expenditure row or any of their child rows
                $is_income_row = stripos($row['title'], 'Income') !== false && $level <= 1;
                $is_sales_row = stripos($row['title'], 'Sales') !== false && $level <= 1;
                $is_expenditure_row = stripos($row['title'], 'Expenditure') !== false && $level <= 1 || 
                                     (isset($first_level_sr) && (string)$first_level_sr === '5' && $level <= 2);
                $is_income_child = $level > 1 && isset($first_level_sr) && (string)$first_level_sr === '4';
                $is_expenditure_child = $level > 1 && isset($first_level_sr) && (string)$first_level_sr === '5';
                $should_skip_rendering = $is_income_row || $is_income_child || $is_sales_row || $is_expenditure_row || $is_expenditure_child;
                
                // Only render row if it's not Income, Sales or their children
                if ($level <= 1 && !$should_skip_rendering) {
                    $html .= '<tr>';
                    $html .= "<td>{$serial_number}</td>";
                    $html .= "<td>{$padding}{$title}</td>";
                    $html .= '<td></td><td></td><td></td>';
                ...
                if ($level === 1) {
                    ...
                    // Process Income rows for calculations (but don't display Total Income)
                    if (stripos($row['title'], 'Sales') !== false) {
                        $sales_net_total += $net;
                    }
                    if (stripos($row['title'], 'Income from Other Activities') !== false) {
                    ...
                    // Calculate Total Income but don't display it
                    if (stripos($row['title'], 'Income') !== false && $level === 1 && !$printed_income_total) {
                        $total_income = $sales_net_total + $income_other_activities_net_total + $other_income_net_total;
                    ...
                    // Calculate Total Expenditure but don't display it
                    static $printed_expenditure_total = false;
            ...
            if ($level === 0 && $net_profit_calculated) {
                ...
                // First row: label only (centered, blue background, white text)
                $html .= '<tr style="background-color:black;"><td colspan="5"></td></tr>';
                $html .= '<tr style="font-weight:bold;">';
                $html .= '<td colspan="5" style="text-align:center; font-size:1.35em; color:black; letter-spacing:1px;"><b>Total Assets = Total Liabilities + Total Capital</b></td>';
                $html .= '</tr>';
                $html .= '<tr style="font-weight:bold; background-color:#198754;">';
                $html .= '<td colspan="5" style="text-align:center; font-size:1.35em; color:black; letter-spacing:1px;">'
                    . '<b>' . number_format($total_assets, 2) . ' = '
                    . number_format($net_total_liabilities, 2) . ' + '
                    . number_format($total_capital, 2) . '</b>'
                    . '</td>';
                $html .= '</tr>';
            }
