                                ...
                                if ( count ( $sales ) > 0 ) {
                                    foreach ( $sales as $sale ) {
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td><?php echo date_setter ( $saleInfo -> date_sale ) ?></td>
                                            <td>
                                                <?php
                                                    echo '<span class="label label-success">Results Added</span><br/><br/>';
                                                    echo 'Added By: ' . ucwords ( get_user ( $results -> user_id ) -> name ) . '<br/>';
                                                    echo date_setter ( $results -> date_added );
                                                ?>
                                            </td>
                                            <td>
                                                <a href="<?php echo base_url ( '/invoices/print-lab-single-invoice/?id=' . @$results -> id . '&sale-id=' . $sale -> sale_id . '&parent-id=' . $sale -> test_id ) ?>"
                                                   target="_blank"
                                                   class="btn green btn-xs">Print</a>
                                                
                                                <a href="<?php echo base_url ( '/invoices/print-lab-single-invoice/?id=' . @$results -> id . '&sale-id=' . $sale -> sale_id . '&parent-id=' . $sale -> test_id . '&logo=true' ) ?>"
                                                   target="_blank"
                                                   class="btn purple btn-xs">L-Print</a>
                                            </td>
                                        </tr>
                                        <?php
                                    }
