                ...
                <div class="portlet-body">
                    ...
                    <form method="post" enctype="multipart/form-data">
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <?php
                                $counter   = 1;
                                $testID    = $this -> input -> get ( 'test-id' );
                                $testInfo  = get_test_by_id ( $testID );
                                $sub_tests = get_active_child_tests_by_parent ( $testID );
                                ...
                                if ( count ( $sales ) > 0 ) {
                                    foreach ( $sales as $sale ) {
                                        ...
                                        <tr class="odd gradeX">
                                            <td align="center">
                                                <?php echo $isParent ? '<strong>' . $counter++ . '</strong>' : $counter++ ?>
                                            </td>
                                            <td>
                                                <?php echo $isParent ? '<strong>' . $test -> name . '</strong>' : $test -> name ?>
                                            </td>
                                            <td>
                                                <?php
                                                    $nil = '';
                                                    if ( count ( $ranges ) > 0 ) {
                                                        foreach ( $ranges as $range ) {
                                                            if ( strtolower ( $range -> start_range ) === 'nil' || strtolower ( $range -> end_range ) === 'nil' )
                                                                $nil = 'NIL';
                                                        }
                                                    }
                                                ?>
                                                <textarea name="result[]" class="form-control" rows="5"
                                                          placeholder="Add result"><?php echo ( !empty( $results ) && !empty( trim ( $results -> result ) ) ) ? $results -> result : $nil ?></textarea>
                                            </td>
                                ...
                                if ( count ( $sub_tests ) > 0 ) {
                                    foreach ( $sub_tests as $sub_test ) {
                                        $results = @get_test_results ( $_REQUEST[ 'sale-id' ], $sub_test -> id );
                                        $unit_id = @get_test_unit_id_by_id ( $sub_test -> id );
                                        $unit    = @get_unit_by_id ( $unit_id );
                                        $ranges  = @get_reference_ranges_by_test_id ( $sub_test -> id );
                                        ?>
                                        <input type="hidden" name="test_id[]" value="<?php echo $sub_test -> id ?>">
                                        <tr class="odd gradeX">
                                            <td></td>
                                            <td style="padding-left: 25px"><?php echo $sub_test -> name ?></td>
                                            <td>
                                                <?php
                                                    $nil = '';
                                                    if ( count ( $ranges ) > 0 ) {
                                            ...
                                            <?php if ( $sub_test -> category == 'covid' ) : ?>
                                                <td>
                                                    <select name="detected[]" class="form-control select2me">
                                                        <option
                                                                value="detected" <?php if ( @$results -> detected == 'detected' )
                                            ...
                                            <td>
                                                <input type="checkbox" name="abnormal[<?php echo $sub_test -> id ?>]"
                                                       class="form-control"
                                                       value="1" <?php echo @$results -> abnormal == '1' ? 'checked="checked"' : '' ?>>
                                                Yes
                                            </td>
                                ...
                                if ( count ( $sales ) > 0 ) {
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <?php if ( !empty( $access ) and in_array ( 'lab_result_verify_and_add_result_button', explode ( ',', $access -> access ) ) ) : ?>
