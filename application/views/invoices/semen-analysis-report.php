...
<table class="items" width="100%" style="font-size: 9pt; border-collapse: collapse; margin-top: 10px; border: 0"
    ...
    <tr style="background: #f5f5f5;">
            <th align="left" style="width: 20%;">Test Name</th>
            <th align="left" style="width: 10%;">Results</th>
            <th colspan="2" align="center" style="width: 30%;">Previous Results</th>
    ...
    <tr>
        <td align="left" colspan="2" width="35%">
            <span style="font-weight: 900; font-size: 14px">
                <?php
                    $previous_results = get_previous_test_results ( $sale_id, SEMEN_ANALYSIS );
                    $testDetails      = get_test_by_id ( SEMEN_ANALYSIS );
    ...
    <?php
        if ( count ( semen_analysis_general ) > 0 ) {
            foreach ( semen_analysis_general as $key => $test_id ) {
    ...
    <?php
        if ( count ( semen_analysis_motility ) > 0 ) {
            foreach ( semen_analysis_motility as $key => $test_id ) {
                $test_info        = get_test_by_id ( $test_id );
                $unit             = get_test_unit_id_by_id ( $test_id );
                $ranges           = get_reference_ranges_by_test_id ( $test_id );
                $result           = get_test_results ( $sale_id, $test_id );
    ...
    <?php
        if ( count ( semen_analysis_count ) > 0 ) {
            foreach ( semen_analysis_count as $key => $test_id ) {
                $test_info        = get_test_by_id ( $test_id );
                $unit             = get_test_unit_id_by_id ( $test_id );
                $ranges           = get_reference_ranges_by_test_id ( $test_id );
                $result           = get_test_results ( $sale_id, $test_id );
        ...
        if ( count ( semen_analysis_cells ) > 0 ) {
            foreach ( semen_analysis_cells as $key => $test_id ) {
                $test_info        = get_test_by_id ( $test_id );
                $unit             = get_test_unit_id_by_id ( $test_id );
                $ranges           = get_reference_ranges_by_test_id ( $test_id );
                $result           = get_test_results ( $sale_id, $test_id );
