    ...
    <div class="col-md-12">
        ...
        <div class="portlet box green">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-globe"></i> MRI Reports
                </div>
            </div>
                    ...
                    <?php
                        ...
                        if ( count ( $reports ) > 0 ) {
                            foreach ( $reports as $report ) {
                                ...
                                $status = get_report_verify_status ( $report -> id, 'hmis_mri' );
                                ?>
                                <tr class="odd gradeX">
                                    ...
                                    <td class="btn-group-xs">
                                        ...
                                        <?php if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'edit_mri_report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                            <a type="button" class="btn blue"
                                               href="<?php echo base_url ( '/radiology/mri/search-mri-report?report_id=' . $report -> id ) ?>">Edit</a>
                                        <?php endif; ?>
                                        
                                        <?php if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'verify_mri_report_button', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) && empty( $status ) ) : ?>
                                            <a type="button" class="btn green"
                                               onclick="return confirm('Are you sure?')"
                                               href="<?php echo base_url ( '/radiology/x-ray/verify-mri-report?report-id=' . $report -> id . '&sale_id=' . $report -> sale_id . '&test_id=' . $report -> test_id ) ?>">Verify</a>
                                        <?php endif; ?>
                                        
                                        <?php if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'delete_mri_report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                            <a type="button" class="btn red"
                                               href="<?php echo base_url ( '/radiology/mri/delete-mri-report/' . $report -> id ) ?>"
