                            ...
                            foreach ( $reports as $report ) {
                                ...
                                <tr class="odd gradeX">
                                    ...
                                    <td class="btn-group-xs">
                                        ...
                                        <?php if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'edit_ultrasound_report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                            <a type="button" class="btn blue"
                                               href="<?php echo base_url ( '/radiology/ultrasound/edit-ultrasound-report?report_id=' . $report -> id ) ?>">Edit</a>
                                        <?php endif; ?>
                                        
                                        <?php if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'verify_ultrasound_report_button', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) && empty( $status ) ) : ?>
                                            <a type="button" class="btn green"
                                               onclick="return confirm('Are you sure?')"
                                               href="<?php echo base_url ( '/radiology/x-ray/verify-ultrasound-report?report-id=' . $report -> id . '&sale_id=' . $report -> sale_id . '&test_id=' . $report -> test_id ) ?>">Verify</a>
                                        <?php endif; ?>
                                        
                                        <?php if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'delete_ultrasound_report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                            <a type="button" class="btn red"
                                               href="<?php echo base_url ( '/radiology/ultrasound/delete-ultrasound-report/' . $report -> id ) ?>"
